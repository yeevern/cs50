...
int main(void)
{
    // List of size 0. We initialize the value to NULL explicitly, so there's no garbage value for our list variable
    node *list = NULL;

    // Allocate memory for a node, n
    node *n = malloc(sizeof(node));
    if (n == NULL)  // check if something went wrong/out of memory
    {
        return 1;
    }
    ...
    list = n;  // list is a pointer; n is the addr of an actual node
    ...
    if (n == NULL)
    {
        free(list);  // no need to free n because n is assign to list
        return 1;
    }
    ...
    for (node *tmp = list; tmp != NULL; tmp = tmp->next)  // node *tmp = list - a temporary pointer to the beginning of the list
    {
        printf("%i\n", tmp->number);
    }
